...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize recordingSize:(CGSize)recordingSize
{
    self.videoRecordingSize = recordingSize;
    return [self initWithMovieURL:newMovieURL size:newSize fileType:AVFileTypeQuickTimeMovie outputSettings:nil];
}
...
- (void)initializeMovieWithOutputSettings:(NSDictionary *)outputSettings;
{
    ...
    if (outputSettings == nil) 
    {
        ...
        [settings setObject:[NSNumber numberWithInt:self.videoRecordingSize.width] forKey:AVVideoWidthKey];
        [settings setObject:[NSNumber numberWithInt:self.videoRecordingSize.height] forKey:AVVideoHeightKey];
        [settings setObject:AVVideoScalingModeResizeAspectFill forKey:AVVideoScalingModeKey];
        [settings setObject:compressionSettings forKey:AVVideoCompressionPropertiesKey];
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    runAsynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        NSLog(@"%zu", CVPixelBufferGetWidth(pixel_buffer));
