...
- (CGRect)frameForToolbarAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone &&
        UIInterfaceOrientationIsLandscape(orientation)) height = 55;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //NSLog(@"%f %f %f", scrollView.contentOffset.x, scrollView.contentSize.width, visibleBounds.size.width);
    
    if (UIInterfaceOrientationIsPortrait(self.interfaceOrientation))
    {
        if (scrollView.contentOffset.x > scrollView.contentSize.width - (3*visibleBounds.size.width)/4)
            [self doneButtonPressed:nil];
    }
    else
    {
        if (scrollView.contentOffset.x > scrollView.contentSize.width - (5*visibleBounds.size.width)/6)
            [self doneButtonPressed:nil];
    }
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    MWPhoto* p = (MWPhoto*)photo;
                    
                    
                    
                    if ([[[p.photoURL scheme] lowercaseString] isEqualToString:@"assets-library"])
                    {
                        dispatch_semaphore_t sema = dispatch_semaphore_create(0);
                        
                        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        ALAssetsLibrary *assetslibrary = [[ALAssetsLibrary alloc] init];
                        [assetslibrary assetForURL:p.photoURL
                         ...
                         
                        {
                                ...
                                if (adjustment)
                                {
                                        ...
                                        if (filterArray && !error)
                                        {
                                            ...
                                            iref = [context createCGImage:image fromRect:[image extent]];
                                        }
                                }
                                ...
                                if (iref)
                                {
                                        ...
                                        items = [NSMutableArray arrayWithObject:img];
