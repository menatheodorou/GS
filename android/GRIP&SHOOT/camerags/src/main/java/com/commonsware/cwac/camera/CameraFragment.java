...
public class CameraFragment extends Fragment {
	...
	private boolean mIsOnFocusing = false;
	...
	public void takePicture(boolean needBitmap, boolean needByteArray, boolean needFile) {
		cameraView.takePicture(needBitmap, needByteArray, needFile);
	}
	...
	public void record(long maxSize, MediaRecorder.OnInfoListener infoListener, MediaRecorder.OnErrorListener errorListener) throws Exception {
		cameraView.record(maxSize, infoListener, errorListener);
	}
	...
	/**
	 * Call this to begin an manual-focus operation (e.g., in response to the user
	 * tapping something to focus the camera).
	 */
	public boolean manualFocus(MotionEvent event) {
		return cameraView.manualFocus(event);
	}
	...
	// Updated by David He to return flash mode which is applied before fragment created.
